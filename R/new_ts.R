...
dgp_vdl <- function(T = 600, m = 2, seed = NULL) {
  ...
  for (t in (init + 1):T) {
    # treatment
    lpA <- 0
    if (m >= 1) lpA <- lpA + coef_A$W1*W1[t-1] + coef_A$W2*W2[t-1] +
        coef_A$Y*Y[t-1]  + coef_A$A*A[t-1]
    if (m >= 2) lpA <- lpA + coef_A$W3_2 * W3[t-2]
    A[t] <- rbinom(1, 1, expit(lpA))
    
    # outcome
    lpY <- coef_Y$intercept + coef_Y$A * A[t]
    if (m >= 1) lpY <- lpY + coef_Y$W1*W1[t-1] + coef_Y$W2*W2[t-1] + coef_Y$W3*W3[t-1]
    if (m >= 2) lpY <- lpY + coef_Y$W1_2*W1[t-2] + coef_Y$W3_2*W3[t-2]
    Y[t] <- rbinom(1, 1, expit(lpY))
